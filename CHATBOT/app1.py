...
def send_feedback_email(name, subject, message):
    sender_email = 'daminmain@gmail.com'  # Replace with your email
    receiver_email = 'pavi.basmalar@gmail.com'  # Replace with recipient's email
    ...
    with smtplib.SMTP(smtp_server, smtp_port) as server:
        server.starttls()
        server.login(sender_email, 'kpqtxqskedcykwjz')  # Replace with your email password
...
def get_response():
    ...
    response_ta = ''  # Tamil response

    for i in data["intents"]:
        if i["tag"] == ints[0]["intent"]:
            responses = i["responses"]
            response_en = random.choice(responses)
            # Assuming you have a function to translate English to Tamil
            response_ta = translate_to_tamil(response_en)
            break

    return render_template('result.html', response_en=response_en, response_ta=response_ta)
...
def send_feedback():
    ...
    send_feedback_email(name, subject, message)
